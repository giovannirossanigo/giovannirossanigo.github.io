...
<div class="definition">
  ...
  <ol>
    <li>
      We define the <em>Cauchy-completion $\Li(\mc{C})$ of $\mc{C}$</em> as the full subcategory of $\Ind(\mc{C})$ spanned by those functors $A:\mc{C}^{\op}\to\Space$ which can be expressed as colimits $E\simeq \colim \yo(a_*)$ of a Cauchy sequence $a_*:\mb{N}^{\leq}\to \mc{C}$.
    </li>
    <li>
      We define the <em>category of compactly supported functors $\CS(\mc{C})$ of $\mc{C}$</em> as the full subcategory of $\Ind(\mc{C})$ spanned by those functors $E:\mc{C}^{\op}\to\Spc$ such that for every $j\in\Z$
      there exists an integer $i>0$ such that
      $\pi_0\Hom_{\PSh(\mc{C})}(\y(\susp^jM_i), E)=0$.
    </li>
    <li>
      We let $\S(\mc{C})= \Li(\mc{C})\cap \CS(\mc{C})$.
    </li>
  </ol>
</div>
